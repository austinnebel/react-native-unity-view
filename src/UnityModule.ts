...
export interface UnityModule {
   ...
   * @param methodName (optional) Method name in GameObject instance.
   */
  postMessageAsync<TResponse = any, TType extends number = UnityMessageType>(
    id: string,
    data: any,
    gameObject?: string,
    methodName?: string,
...
class UnityModuleImpl implements UnityModule {
  ...
  >(
    ...
    return new Observable<TResponse>(
      (subscriber: Subscriber<TResponse>): TeardownLogic => {
  ...
  public addUnityMessageListener(
    listener: (message: UnityMessage) => void,
  ): number {
    const id = this.getHandleId();
    this.unityMessageListeners[id] = listener;
    return id;
  ...
  public addUnityRequestListener(
    listener: (handler: UnityRequestHandler) => void,
  ): number {
    const id = this.getHandleId();
    this.unityRequestListeners[id] = listener;
  ...
  private handleMessage(
    message: string,
  ): string | UnityMessage | UnityRequestHandler | undefined {
      ...
      if (unityMessage.isRequestCompletion()) {
          ...
          if (unityMessage.isResponse()) {
            if (__DEBUG_UNITY_VIEW__) {
              const time = Math.round(
                nativePerformanceNow() - awaitEntry.startedAt,
              );
              console.log(
                `RESPONSE ${unityMessage.uuid} (${time}ms): ` +
                  message.substr(UnityMessagePrefix.length),
              );
            }
        ...
        if (__DEBUG_UNITY_VIEW__) {
          if (unityMessage.isRequest()) {
            console.log(
              'INCOMMING REQUEST' + message.substr(UnityMessagePrefix.length),
            );
          } else {
